...
substitutes its ``${FIXTURES_DIR}``/``${JUVENAL_URL}``/``${DRUID_URL}`` placeholders
...
example's README, plus a fourth passthrough case for a non-matching Druid query.
"""
...
def server(tmp_path, upstream, mockstack_server):
    ...
    rendered = string.Template((EXAMPLE_DIR / "rules.yml").read_text()).safe_substitute(
        ...
        JUVENAL_URL=upstream.base_url,
        DRUID_URL=upstream.base_url,
    )
...
def test_stamped_get_project_returns_fixture(server, upstream):
    ...
    r = httpx.get(f"{server.base_url}/juvenal/api/v2/project/proj-123", headers=STAMPED)
    assert r.status_code == 200
    assert r.headers["x-mockstack-result"] == "template"
    assert r.headers["x-mockstack-rule"] == "juvenal-project-eval"
    assert r.json() == {
        "id": "proj-123",
...
def test_unstamped_get_project_falls_through_to_upstream(server, upstream):
    ...
    r = httpx.get(f"{server.base_url}/juvenal/api/v2/project/proj-123")
    assert r.status_code == 200
    assert r.headers["x-mockstack-result"] == "proxy"
    assert r.headers["x-mockstack-rule"] == "juvenal-passthrough"
    assert r.json()["source"] == "upstream"
    assert len(upstream.calls) == 1
    assert upstream.calls[0]["path"] == "/api/v2/project/proj-123"


def test_stamped_druid_pricing_query_returns_fixture(server, upstream):
    """README scenario 3: stamped Druid POST mentioning pricing_facts -> template fixture."""
    sql = "SELECT client_id, SUM(amount)\nFROM pricing_facts WHERE 1=1"
    r = httpx.post(
        f"{server.base_url}/druid/druid/v2/sql",
        json={"query": sql},
        headers=STAMPED,
    )
    assert r.status_code == 200
    assert r.headers["x-mockstack-result"] == "template"
    assert r.headers["x-mockstack-rule"] == "druid-pricing-eval"
    assert r.json() == [
...
def test_stamped_druid_non_pricing_query_falls_through_to_upstream(server, upstream):
    """Fourth scenario: stamped Druid POST that doesn't mention pricing_facts -> proxy."""
    r = httpx.post(
        f"{server.base_url}/druid/druid/v2/sql",
    ...
    assert r.status_code == 200
    assert r.headers["x-mockstack-result"] == "proxy"
    assert r.headers["x-mockstack-rule"] == "druid-passthrough"
    assert r.json()["source"] == "upstream"
    assert len(upstream.calls) == 1
