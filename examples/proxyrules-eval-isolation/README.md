...
```bash
# 1. Stamped GET -> served from fixtures/healthy/juvenal/project.json.j2
curl -i -H "X-Request-Eval-Scenario: healthy" \
  http://127.0.0.1:8000/juvenal/api/v2/project/proj-123
# X-Mockstack-Result: template
# X-Mockstack-Rule: juvenal-project-eval

# 2. Unstamped GET -> reverse-proxied to upstream.py
curl -i http://127.0.0.1:8000/juvenal/api/v2/project/proj-123
# X-Mockstack-Result: proxy
# X-Mockstack-Rule: juvenal-passthrough

# 3. Stamped Druid POST mentioning pricing_facts -> served from fixtures
curl -i -H "X-Request-Eval-Scenario: healthy" -H "Content-Type: application/json" \
  -d '{"query": "SELECT client_id FROM pricing_facts WHERE 1=1"}' \
  http://127.0.0.1:8000/druid/druid/v2/sql
# X-Mockstack-Result: template
# X-Mockstack-Rule: druid-pricing-eval
```

`mockstack/tests/live/test_example_eval_isolation.py` runs these same three
scenarios (plus a fourth: a stamped Druid query that does *not* mention
`pricing_facts`, which falls through to the passthrough rule) against this
